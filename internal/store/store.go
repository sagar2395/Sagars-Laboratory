...
// Package store is Flightdeck's durable state: runs, their logs and steps, and
// the audit trail.
//
...
// DefaultPath returns the database location: $FLIGHTDECK_HOME/flightdeck.db,
// defaulting to ~/.flightdeck/flightdeck.db.
func DefaultPath() (string, error) {
	if home := os.Getenv("FLIGHTDECK_HOME"); home != "" {
		return filepath.Join(home, "flightdeck.db"), nil
	}
	dir, err := os.UserHomeDir()
	if err != nil {
		return "", fmt.Errorf("locating home directory (set FLIGHTDECK_HOME to override): %w", err)
	}
	return filepath.Join(dir, ".flightdeck", "flightdeck.db"), nil
}
...
func (s *Store) migrate(ctx context.Context) error {
	...
	if applied > latest {
		return fmt.Errorf("%w: %s is at schema %d, this build knows %d — "+
			"upgrade labctl, or point FLIGHTDECK_HOME at a different directory",

...
func TestDefaultPath(t *testing.T) {
	t.Run("honours FLIGHTDECK_HOME", func(t *testing.T) {
		t.Setenv("FLIGHTDECK_HOME", "/custom/home")
		got, err := DefaultPath()
		if err != nil {
			t.Fatalf("DefaultPath: %v", err)
		}
		if want := filepath.Join("/custom/home", "flightdeck.db"); got != want {
			t.Errorf("got %q, want %q", got, want)
		}
	})

	t.Run("falls back to the home directory", func(t *testing.T) {
		t.Setenv("FLIGHTDECK_HOME", "")
		got, err := DefaultPath()
		if err != nil {
			t.Skipf("no home directory in this environment: %v", err)
		}
		if !strings.HasSuffix(got, filepath.Join(".flightdeck", "flightdeck.db")) {
			t.Errorf("got %q, want a path ending in .flightdeck/flightdeck.db", got)
		}
	})
}

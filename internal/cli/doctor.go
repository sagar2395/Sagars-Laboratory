...
import (
	...
	"go.flightdeck.dev/flightdeck/internal/toolchain"
)
...
func doctorCmd() *cobra.Command {
	return &cobra.Command{
		Use:   "doctor",
		Short: "Check that your environment can run Flightdeck",
		Long: `Verifies every external tool Flightdeck depends on: that it is
...
func runDoctor(ctx context.Context, out io.Writer, runner toolchain.Runner) error {
	...
	fmt.Fprintln(out, "Flightdeck environment check")
	fmt.Fprintln(out)
	...
	fmt.Fprintln(out, "\n✓ Everything Flightdeck needs is installed and current.")
	return nil
}

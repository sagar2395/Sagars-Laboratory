...
func TestExecRun(t *testing.T) {
	...
	t.Run("inherits the parent environment as well", func(t *testing.T) {
		t.Setenv("FLIGHTDECK_TEST_INHERITED", "yes")
		script := writeScript(t, `echo "${FLIGHTDECK_TEST_INHERITED:-unset}"`)
		var stdout bytes.Buffer
...
func TestExecLookPath(t *testing.T) {
	...
	t.Run("fails for a missing binary", func(t *testing.T) {
		if _, err := e.LookPath("flightdeck-definitely-not-a-real-binary"); err == nil {
			t.Fatal("expected an error for a missing binary")
		}
	})
}

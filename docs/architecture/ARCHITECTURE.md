...
```
go.mod                          module go.flightdeck.dev/flightdeck
...
Long scripts emit step markers (`##flightdeck:step:<name>`) that the engine
...
Database lives at `~/.flightdeck/flightdeck.db` (override with
`FLIGHTDECK_HOME`). Schema is versioned with embedded, forward-only migrations
...
- **`FLIGHTDECK_CONTENT_PATH`** adds external content roots, so a team can keep
...
## 10. Observability of Flightdeck itself

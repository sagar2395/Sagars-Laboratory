...
- A scratch `FLIGHTDECK_HOME` so this runbook never touches your real history:

```bash
$ export FLIGHTDECK_HOME=/tmp/flightdeck-r01
$ mkdir -p "$FLIGHTDECK_HOME"
```
...
```bash
$ ls -la "$FLIGHTDECK_HOME"
```

**Expect:** `flightdeck.db` plus WAL sidecar files (`-wal`, `-shm`). Those
sidecars are how a reader proceeds while a write is in flight.

**Failure signature — a permissions error:** `FLIGHTDECK_HOME` points somewhere
...
#!/usr/bin/env bash
set -euo pipefail
echo "##flightdeck:step:phase-one"
echo "starting work"
...
open a second terminal (remember to export `FLIGHTDECK_HOME` there too).
...
$ rm -rf "$FLIGHTDECK_HOME"
$ unset FLIGHTDECK_HOME
...
|---|---|---|---|
| 1 | Database created at `FLIGHTDECK_HOME` | | |

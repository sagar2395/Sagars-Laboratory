...
`go.flightdeck.dev/labctl` with public packages at
`go.flightdeck.dev/labctl/pkg/checks`. Tooling (coverage, lint, vulnerability
...
1. Move `go.mod` to the repo root, module `go.flightdeck.dev/flightdeck`.

...
func TestList_CloudRuntimeWithExplicitContext(t *testing.T) {
	root := t.TempDir()
	rtDir := filepath.Join(root, "runtimes")
	makeRuntime(t, rtDir, "aks", []string{
		"INGRESS_CLASS=nginx",
		"KUBECONFIG_CONTEXT=my-aks-cluster",
	})

	m := newTestManager(t, rtDir, "sagars-cluster", "my-aks-cluster", []string{"my-aks-cluster"})
...
func TestList_CloudRuntimeNoContextConfig(t *testing.T) {
	root := t.TempDir()
	rtDir := filepath.Join(root, "runtimes")
	// aks runtime.env has no KUBECONFIG_CONTEXT
	makeRuntime(t, rtDir, "aks", []string{
		"INGRESS_CLASS=nginx",
		"STORAGE_CLASS=managed-csi",
	})

	// Even if some context named "aks" happens to be in kubeconfig, it must
	// NOT be treated as active — we cannot reliably guess the context name.
	m := newTestManager(t, rtDir, "sagars-cluster", "aks", []string{"aks"})
...
func TestList_MultipleRuntimes(t *testing.T) {
	...
	rtDir := filepath.Join(root, "runtimes")
	makeRuntime(t, rtDir, "k3d", nil)
	makeRuntime(t, rtDir, "aks", []string{"INGRESS_CLASS=nginx"}) // no KUBECONFIG_CONTEXT
	...
	aks, ok := byName["aks"]
	if !ok {
		t.Fatal("aks not in list")
	}
	if aks.Active || aks.Current {
		t.Errorf("aks: want Active=false Current=false, got Active=%v Current=%v", aks.Active, aks.Current)
	}
}

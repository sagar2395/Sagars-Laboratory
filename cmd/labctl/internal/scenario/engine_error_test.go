...
// denyAllEntitlement is a test entitlement that denies every request.
type denyAllEntitlement struct{}

func (denyAllEntitlement) Authorize(_ context.Context, req entitlement.Request) error {
	return entitlement.Denied(req.Name, "test: deny-all")
}
...
func TestUp_EntitlementDenied_ReturnsError(t *testing.T) {
	root := t.TempDir()
